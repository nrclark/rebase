...
int load_image_info() {
  ...
  if (ret == 0 && verbose) {
    printf("== read %" PRIu64 " (0x%08" PRIx64
           ") bytes (database w/o strings)\n",
  ...
  if (ret == 0) {
    ...
    for (i = 0; i < img_info_size; ++i) {
      ...
      if (verbose)
        printf("%03d: base 0x%0*" PRIx64
               " size 0x%08x slot 0x%08x namesize %4d %c\n",
               i, hdr.machine == IMAGE_FILE_MACHINE_I386 ? 8 : 12,
  ...
  if (ret == 0) {
    ...
    for (i = 0; i < img_info_size; ++i) {
      img_info_list[i].name =
          (char *)calloc(img_info_list[i].name_size, sizeof(char));
      ...
      if (read(fd, (void *)img_info_list[i].name,
        ...
        printf("%03d: namesize %4d (0x%04x) %s\n", i,
               (uint32_t)img_info_list[i].name_size,
               (uint32_t)img_info_list[i].name_size, img_info_list[i].name);
      }
    }
  }
  close(fd);
...
void usage() {
  fprintf(stderr, "usage: %s [-hqvV] dbfile\n"
