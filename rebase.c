 ...
 *
 * $Id: rebase.c,v 1.30 2008/06/07 20:37:48 jt Exp jt $
 */
...
void gen_progname(const char *arg0) {
  char *p;
...
int main(int argc, char *argv[]) {
  ...
  if (machine == IMAGE_FILE_MACHINE_I386) {
    GetImageInfos64("/bin/msys-1.0.dll", NULL, &cygwin_dll_image_base,
  ...
  if (machine == IMAGE_FILE_MACHINE_I386) {
    ...
    GetImageInfos64("/bin/cygwin1.dll", NULL, &cygwin_dll_image_base,
                    &cygwin_dll_image_size);
...
int save_image_info() {
  ...
  /* Write the list. */
    ...
    /* Write all strings. */
    for (i = 0; i < img_info_size; ++i)
      if (write(fd, img_info_list[i].name, strlen(img_info_list[i].name) + 1) <
          0) {
        fprintf(stderr, "%s: failed to write rebase database: %s\n", progname,
                strerror(errno));
        ret = -1;
  ...
  if (ret < 0)
    ...
    if (unlink(db_file) < 0 && errno != ENOENT) {
      fprintf(stderr, "%s: failed to remove old rebase database file \"%s\":\n"
                      "%s\n"
              ...
              progname, db_file, strerror(errno), tmp_file, db_file, tmp_file,
              db_file);
      ret = -1;
      ...
      fprintf(stderr, "%s: failed to rename \"%s\" to \"%s\":\n"
                      ...
                      "otherwise the new rebase database will be unusable.\n",
              progname, tmp_file, db_file, strerror(errno), tmp_file, db_file);
      ret = -1;
    }
  }
  return ret;
...
int load_image_info() {
  ...
  if (hdr.machine != machine) {
    if (hdr.machine == IMAGE_FILE_MACHINE_I386)
      fprintf(stderr, "%s: \"%s\" is a database file for 32 bit DLLs but\n"
                      "I'm started to handle 64 bit DLLs.  If you want to "
                      "handle 32 bit DLLs,\n"
                      "use the -4 option.\n",
              progname, db_file);
    else if (hdr.machine == IMAGE_FILE_MACHINE_AMD64)
      fprintf(stderr, "%s: \"%s\" is a database file for 64 bit DLLs but\n"
                      "I'm started to handle 32 bit DLLs.  If you want to "
                      "handle 64 bit DLLs,\n"
                      "use the -8 option.\n",
              progname, db_file);
    else
  ...
  if (ret == 0) {
    ...
    img_info_list = (img_info_t *)calloc(img_info_max_size, sizeof(img_info_t));
    if (!img_info_list) {
  ...
  if (ret == 0) {
    for (i = 0; i < img_info_size; ++i) {
      img_info_list[i].name = (char *)malloc(img_info_list[i].name_size);
      ...
      if ((read_ret =
               read(fd, img_info_list[i].name, img_info_list[i].name_size)) !=
...
static BOOL set_cannot_rebase(img_info_t *img) {
...
int merge_image_info() {
  ...
  if (img_info_rebase_start) {
    for (i = img_info_rebase_start; i < img_info_size; ++i) {
      ...
      match = bsearch(&img_info_list[i], img_info_list, img_info_rebase_start,
                      sizeof(img_info_t), img_info_name_cmp);
      if (match) {
  ...
  for (i = 0; i < img_info_rebase_start; ++i) {
    ...
    if (access(img_info_list[i].name, F_OK) == -1 ||
        !GetImageInfos64(img_info_list[i].name, NULL, &cur_base, &cur_size)) {
      free(img_info_list[i].name);
      memmove(img_info_list + i, img_info_list + i + 1,
              (img_info_size - i - 1) * sizeof(img_info_t));
    ...
    if (set_cannot_rebase(&img_info_list[i]))
      ...
      /* Does the previous DLL reach into the address space of this
         DLL?  This happens if the previous DLL is not rebaseable. */
               ...
               cur_base <
                   img_info_list[i - 1].base + img_info_list[i + 1].slot_size)
        img_info_list[i].base = 0;
  ...
  while (img_info_list[0].base == 0) {
    ...
    for (i = 0, new_base = 0; img_info_list[i].base == 0; ++i, new_base = 0) {
      ...
      if (new_base >= img_info_list[end].base + img_info_list[end].slot_size
#if defined(__CYGWIN__) || defined(__MSYS__)
          /* Don't overlap the Cygwin/MSYS DLL. */
          &&
          (new_base >= cygwin_dll_image_base + cygwin_dll_image_size ||
           new_base + img_info_list[i].slot_size <= cygwin_dll_image_base)
#endif
              )
        break;
    }
...
BOOL collect_image_info(const char *pathname) {
  ...
  if (img_info_size >= img_info_max_size) {
    ...
    img_info_list = (img_info_t *)realloc(
        img_info_list, img_info_max_size * sizeof(img_info_t));
    if (!img_info_list) {
      fprintf(stderr, "%s: Out of memory.\n", progname);
      return FALSE;
    }
...
void print_image_info() {
  ...
  for (i = 0; i < img_info_size; ++i) {
    ...
    for (tst = i + 1; tst < img_info_size && img_info_list[tst].base < end;
         ++tst) {
      img_info_list[i].flag.needs_rebasing = 1;
      img_info_list[tst].flag.needs_rebasing = 1;
    }
    printf("%-*s base 0x%0*" PRIx64 " size 0x%08x %c\n", name_width,
           img_info_list[i].name, machine == IMAGE_FILE_MACHINE_I386 ? 8 : 12,
           (uint64_t)img_info_list[i].base, (uint32_t)img_info_list[i].size,
...
retry:
  ...
  if (down_flag && *new_image_base >= cygwin_dll_image_base &&
...
void usage() {
  fprintf(stderr, "usage: %s [-b BaseAddress] [-o Offset] [-48dOsvV]"
                  " [-T [FileList | -]] Files...\n"
