 ...
 *
 * $Id: rebaseimage.cc,v 1.7 2003/02/17 22:35:54 habacker Exp $
 */
...
BOOL ReBaseImage64(LPCSTR CurrentImageName,
  ...
  if (!dll.checkRelocations()) {
    if (Base::debug)
      std::cerr
          << "error: dll relocation errors - please fix the errors at first"
          << std::endl;
...
BOOL ReBaseImage(LPCSTR CurrentImageName,
  ...
  BOOL ret =
      ReBaseImage64(CurrentImageName, SymbolPath, fReBase, fRebaseSysfileOk,
                    fGoingDown, CheckImageSize, OldImageSize, &old_base,
                    NewImageSize, &new_base, TimeStamp);
