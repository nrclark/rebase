...
Section::Section(void *aFileBase, SectionHeader *p) {
  header = p;
  adjust = (uintptr_t)header->PointerToRawData + (uintptr_t)aFileBase -
...
void Section::debugprint(const char *title) {
  std::cerr << std::setw(10) << std::setfill(' ') << title
            ...
            << " base: 0x" << std::setw(8) << std::setfill('0') << std::hex
            << header->VirtualAddress << std::dec << " size: 0x" << std::setw(8)
            << std::setfill('0') << std::hex << header->SizeOfRawData
            << std::dec << " file offset: 0x" << std::setw(8)
            << std::setfill('0') << std::hex << header->PointerToRawData
            << std::dec << " offset: 0x" << std::setw(8) << std::setfill('0')
            << std::hex << adjust << std::dec << std::endl;
}
...
bool Section::isIn(Section &in) {
  return header->VirtualAddress >= in.header->VirtualAddress &&
         header->VirtualAddress <
             in.header->VirtualAddress + in.header->SizeOfRawData;
}

bool Section::isIn(uint addr) {
  return addr >= header->VirtualAddress &&
         addr < header->VirtualAddress + header->SizeOfRawData;
}
...
Exports::Exports(SectionList &sections, DataDirectory *iddp) {
  ...
  if (sec) {
    ...
    exports = 0;
    adjust = 0;
    //  std::cerr << __FUNCTION__ << " - error: can't find section for creating
    //  export object" << std::endl;
  }
}
...
void Exports::reset(void) { iterator = 0; }
...
Imports::Imports(SectionList &sections, DataDirectory *iddp) {
  ...
  if (sec) {
    ...
    adjust = 0;
    //  std::cerr << __FUNCTION__ << " - error: can't find section for creating
    //  import object" << std::endl;
  }
}

void Imports::reset(void) { iterator = imports; }
...
void Imports::dump(const char *title) {
  ...
  while ((p = getNextDescriptor()) != NULL) {
    ...
    std::cout << "vma:           Hint     Time      Forward  DLL       First"
              << std::endl;
    std::cout << "               Table    Stamp     Chain    Name      Thunk"
              << std::endl;
    std::cout << std::setw(8) << std::setfill('0') << std::hex << 0 << std::dec
              << "       ";
    std::cout << std::setw(8) << std::setfill('0') << std::hex
              << p->OriginalFirstThunk << std::dec << " ";
    std::cout << std::setw(8) << std::setfill('0') << std::hex
              << p->TimeDateStamp << std::dec << "  ";
    std::cout << std::setw(8) << std::setfill('0') << std::hex
              << p->ForwarderChain << std::dec << " ";
    std::cout << std::setw(8) << std::setfill('0') << std::hex << p->Name
              << std::dec << "  ";
    std::cout << std::setw(8) << std::setfill('0') << std::hex << p->FirstThunk
              << std::dec << std::endl;
  }
  std::cout << std::endl;
}
...
bool Relocations::check(void) {
  ...
  for (; &relocp->SizeOfBlock < (PDWORD)((char *)relocs + size) &&
             relocp->SizeOfBlock != 0;
       relocp =
           (PIMAGE_BASE_RELOCATION)((char *)relocp + relocp->SizeOfBlock)) {
    int NumOfRelocs =
        (relocp->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) / sizeof(WORD);
    ...
    if (!cursec) {
      if (debug)
        std::cerr << "warning: dll is corrupted - relocations for '0x"
                  << std::setw(8) << std::setfill('0') << std::hex << va
                  << std::dec << "' are pointing to a non existent section"
                  << std::endl;
      errors++;
      continue;
    }
    if (debug)
      for (int i = 0; i < NumOfRelocs; i++, p++) {
        ...
        std::cerr << "debug: location= 0x" << std::setw(8) << std::setfill('0')
                  << std::hex << location << std::endl;
      }
  }
  return errors == 0;
}
...
bool Relocations::relocate(int64_t difference) {
  ...
  for (; &relocp->SizeOfBlock < (PDWORD)((char *)relocs + size) &&
    ...
    PWORD p = (PWORD)((uintptr_t)relocp + sizeof(IMAGE_BASE_RELOCATION));
    if (debug) {
      std::cerr << "VirtAdress: 0x" << std::setw(8) << std::setfill('0')
                << std::hex << relocp->VirtualAddress << std::dec << std::endl;
    ...
    if (!cursec) {
      if (debug)
        std::cerr
            << "warning: dll is corrupted - the relocations '0x" << std::setw(8)
            << std::setfill('0') << std::hex << va << std::dec
            << "' points to a non existing section and could not be relocated"
            << std::endl;
      return false;
    } else if (debug)
    ...
    for (int i = 0; i < NumOfRelocs; i++, p++) {
      ...
      case IMAGE_REL_BASED_HIGHLOW: {
        if (debug) {
          std::cerr << "HIGHLOW: 0x" << std::setw(8) << std::setfill('0')
                    << std::hex << location << std::dec << " - ";
          std::cerr << "0x" << std::setw(8) << std::setfill('0') << std::hex
                    << location + adjust + 3 << std::dec << std::endl;
        }
      ...
      case IMAGE_REL_BASED_DIR64: {
        if (debug) {
          std::cerr << "  DIR64: 0x" << std::setw(16) << std::setfill('0')
                    << std::hex << location << std::dec << " - ";
          std::cerr << "0x" << std::setw(16) << std::setfill('0') << std::hex
                    << location + adjust + 7 << std::dec << std::endl;
        }
