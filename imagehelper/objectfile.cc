 ...
 *
 * $Id: objectfile.cc,v 1.11 2003/01/04 12:43:00 habacker Exp $
 */
...
static PCWSTR Win32Path(const char *s) {
...
ObjectFile::ObjectFile(const char *aFileName, bool writeable) {
  ...
  // not found, try with PATH env
    ...
    for (s = strtok(path, ":"); s; s = strtok(NULL, ":")) {
      ...
      hfile = CreateFileW(
          win32_path, writeable ? GENERIC_READ | GENERIC_WRITE : GENERIC_READ,
          FILE_SHARE_READ, NULL, OPEN_EXISTING,
          FILE_ATTRIBUTE_NORMAL | FILE_FLAG_BACKUP_SEMANTICS, NULL);
      // found
      if (hfile != INVALID_HANDLE_VALUE)
  ...
  hfilemapping = CreateFileMapping(
      hfile, NULL, writeable ? PAGE_READWRITE : PAGE_READONLY, 0, 0, NULL);
  if (hfilemapping == 0) {
    CloseHandle(hfile);
    Error = 2;
    return;
  ...
  lpFileBase = MapViewOfFile(
      hfilemapping, writeable ? FILE_MAP_WRITE : FILE_MAP_READ, 0, 0, 0);
  if (lpFileBase == 0) {
  ...
  ImageBase = is64bit_img ? getNTHeader64()->OptionalHeader.ImageBase
...
LinkedObjectFile::LinkedObjectFile(const char *aFileName, bool writable)
  ...
  if (edata)
    ...
    DataDirectory *dir;
    if (is64bit())
      dir = (DataDirectory *)&ntheader64->OptionalHeader
                .DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT];
    else
      dir = (DataDirectory *)&ntheader32->OptionalHeader
                .DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT];
...
bool LinkedObjectFile::rebind(ObjectFileList &cache) {
  if (!isWritable) {
    if (debug)
      std::cerr << __FUNCTION__ << ": error - the objectfile is not writeable"
                << std::endl;
    return false;
  }
  ...
  while ((p = imports->getNextDescriptor()) != NULL) {
    ...
    PIMAGE_THUNK_DATA hintArray =
        PIMAGE_THUNK_DATA((uint)p->OriginalFirstThunk + imports->getAdjust());
    ...
    if ((autoImportFlag = text->isIn((uint)p->FirstThunk)))
      ...
      firstArray =
          PIMAGE_THUNK_DATA((uint)p->FirstThunk + imports->getAdjust());
    ...
    for (; hintArray->u1.Function; hintArray++, firstArray++) {
      PIMAGE_IMPORT_BY_NAME a = PIMAGE_IMPORT_BY_NAME(
          (uint)hintArray->u1.AddressOfData + idata->getAdjust());
      ...
      if (debug)
        std::cerr << "patch_address 0x" << std::setw(8) << std::setfill('0')
                  << std::hex << patch_address << std::dec << " content 0x"
                  << std::setw(8) << std::setfill('0') << std::hex
                  << *patch_address << std::dec << std::endl;
  ...
  if (is64bit()) {
    bdp = (DataDirectory *)&ntheader64->OptionalHeader
              .DataDirectory[IMAGE_DIRECTORY_ENTRY_BOUND_IMPORT];
    first_section = (SectionHeader *)(ntheader64 + 1);
    bp_org = (BoundImportDescriptor *)(&first_section[ntheader64->FileHeader
                                                          .NumberOfSections]);
  } else {
    bdp = (DataDirectory *)&ntheader32->OptionalHeader
              .DataDirectory[IMAGE_DIRECTORY_ENTRY_BOUND_IMPORT];
    first_section = (SectionHeader *)(ntheader32 + 1);
    bp_org = (BoundImportDescriptor *)(&first_section[ntheader32->FileHeader
                                                          .NumberOfSections]);
  }
...
bool LinkedObjectFile::PrintDependencies(ObjectFileList &cache) {
  ...
  if (!isPrinted) {
    if (level == 0)
      ...
      std::cout << filler + strlen(filler) - level * 2 << getFileName()
                << std::endl;
  } else {
    isPrinted = true;
    return true;
