 ...
 *
 * $Id: getimageinfos.cc,v 1.7 2003/01/03 23:16:05 habacker Exp $
 */
...
BOOL GetImageInfos64(LPCSTR filename, WORD *machine, ULONG64 *ImageBase,
                     ULONG *ImageSize) {
  ...
  if (Base::debug)
    std::cerr << "ImageBase: 0x" << std::hex << *ImageBase << " ImageSize: 0x"
              << std::hex << *ImageSize << std::endl;
