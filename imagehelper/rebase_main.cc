 ...
 *
 * $Id: rebase_main.cc,v 1.7 2003/01/03 22:44:56 habacker Exp $
 */
...
int main(int argc, char *argv[]) {
  ...
  for (int i = theArgsIndex; i < argc; i++) {
    ...
    if (theListFlag) {
      ULONG64 ImageBase;
      ULONG ImageSize;
      GetImageInfos64(const_cast<LPSTR>(aFile.c_str()), NULL, &ImageBase,
                      &ImageSize);
      cout << aFile << ": "
           << "ImageBase: 0x" << hex << ImageBase << " ImageSize: 0x" << hex
           << ImageSize << endl;
      ...
      if (aStatus2 != 0) {
        cerr << "ReBaseImage(" << aFile.c_str() << "," << hex << aNewImageBase
             << ") failed with last error = " << dec << aStatus2 << ": "
             << strerror(aStatus2) << endl;
        exit(2);
      }
...
void Usage() {
  ...
  cerr << "usage: rebase [-D] -b BaseAddress [-d] -o Offset <file> ...  rebase "
          "<file>  ([-D] print debug infos)" << endl;
  cerr << "usage: rebase [-D] -l <file> ...        list Imagebase and -size of "
          "<file>" << endl;
