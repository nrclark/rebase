...
typedef enum { DONT_HANDLE = 0, DO_READ = 1, DO_WRITE = 2 } do_handle_t;
...
int get_characteristics(const pe_file *pep, WORD *coff_characteristics,
                        WORD *pe_characteristics);
int set_coff_characteristics(const pe_file *pep, WORD coff_characteristics);
int set_pe_characteristics(const pe_file *pep, WORD pe_characteristics);

static void display_flags(const char *field_name, const symbolic_flags_t *syms,
                          WORD show_symbolic, WORD old_flag_value,
                          WORD new_flag_value);
static char *symbolic_flags(const symbolic_flags_t *syms, long show,
                            long value);
static void append_and_decorate(char **str, int is_set, const char *name,
                                int len);
static void *xmalloc(size_t num);
#define XMALLOC(type, num) ((type *)xmalloc((num) * sizeof(type)))
static void handle_coff_flag_option(const char *option_name,
                                    const char *option_arg, WORD flag_value);
static void handle_pe_flag_option(const char *option_name,
                                  const char *option_arg, WORD flag_value);
static void handle_num_option(const char *option_name, const char *option_arg,
                              int option_index);
...
int main(int argc, char *argv[]) {
  ...
  if (file_list) {
    ...
    if (files_attempted == 0) {
      /* warn the user */
      fprintf(
          stderr, "Error: Could not find any filenames in %s\n",
...
int do_mark(const char *pathname) {
  ...
  pe_file *pep =
      pe_open(pathname, mark_any != 0 || handle_any_sizeof == DO_WRITE);
  if (!pep) {
    fprintf(stderr, "%s: skipped because could not open\n", pathname);
    return 0;
  }

  get_characteristics(pep, &old_coff_characteristics, &old_pe_characteristics);
  ...
  is_executable =
      ((new_coff_characteristics & IMAGE_FILE_EXECUTABLE_IMAGE) > 0);
  ...
  if (!is_executable || is_dll) {
    if (verbose && (new_pe_characteristics &
  ...
  if (mark_any) {
    ...
    if (!is_executable || is_dll) {
      if ((new_pe_characteristics &
           IMAGE_DLLCHARACTERISTICS_TERMINAL_SERVER_AWARE) &&
          !(old_pe_characteristics &
            IMAGE_DLLCHARACTERISTICS_TERMINAL_SERVER_AWARE)) {
        fprintf(stderr, "Warning: setting tsaware on non-executable (%s).\n",
                pathname);
      }
    }
  ...
  if (verbose || !mark_any || coff_characteristics_show ||
      pe_characteristics_show || handle_any_sizeof != DONT_HANDLE) {
    BOOL printed_characteristic = FALSE;
    int i;

    printf("%s: ", pathname);
    if (verbose || (!mark_any && handle_any_sizeof == DONT_HANDLE) ||
      ...
      display_flags("coff", coff_symbolic_flags,
                    ...
                    old_coff_characteristics, new_coff_characteristics);
      display_flags("pe", pe_symbolic_flags,
                    pe_characteristics_show ?: verbose ? old_pe_characteristics
                                                       : 0,
                    old_pe_characteristics, new_pe_characteristics);
      puts("");
    ...
    for (i = 0; i < NUM_SIZEOF_VALUES; ++i) {
      if (sizeof_vals[i].handle != DONT_HANDLE) {
        printf("%*s%-24s: %" PRIu64 " (0x%" PRIx64 ") %s\n",
               printed_characteristic ? (int)strlen(pathname) + 2 : 0, "",
               sizeof_vals[i].name, (uint64_t)sizeof_vals[i].value,
               (uint64_t)sizeof_vals[i].value, sizeof_vals[i].unit);
        printed_characteristic = TRUE;
      }
    }
  }
...
static void display_flags(const char *field_name, const symbolic_flags_t *syms,
  ...
  if (show_symbolic) {
    if (old_flag_value != new_flag_value) {
      ...
      printf("%s(0x%04x%s==>0x%04x%s) ", field_name, old_flag_value,
             old_symbolic, new_flag_value, new_symbolic);
      free(old_symbolic);
      free(new_symbolic);
    } else {
      char *old_symbolic = symbolic_flags(syms, show_symbolic, old_flag_value);
      printf("%s(0x%04x%s) ", field_name, old_flag_value, old_symbolic);
      free(old_symbolic);
    }
  } else {
    if (old_flag_value != new_flag_value)
      printf("%s(0x%04x==>0x%04x) ", field_name, old_flag_value,
             new_flag_value);
    else
...
static char *symbolic_flags(const symbolic_flags_t *syms, long show,
  ...
  for (i = 0; syms[i].name; i++) {
    if (syms[i].flag & show) {
      append_and_decorate(&rVal, (value & syms[i].flag), syms[i].name,
                          syms[i].len);
    }
  }
  if (rVal) {
...
void parse_args(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    if (option_index == 0 && c != long_options[0].val) {
      for (option_index = 1; long_options[option_index].name; ++option_index)
    ...
    case 'd':
      handle_pe_flag_option(long_options[option_index].name, optarg,
    ...
    case 'W':
      ...
      break;
    case 'w':
      handle_coff_flag_option(long_options[option_index].name, optarg,
    ...
    case 'S':
      ...
      break;
    case 'x':
      handle_num_option(long_options[option_index].name, optarg,
  ...
  args_index = optind;
  mark_any = pe_characteristics_set | pe_characteristics_clr |
             coff_characteristics_set | coff_characteristics_clr;
}
...
void *mmap(void *addr, size_t length, int prot, int flags, int fd,
  ...
  if (!h)
    return MAP_FAILED;
  addr = MapViewOfFileEx(h, prot ? FILE_MAP_WRITE : FILE_MAP_READ, 0, 0, 4096,
                         addr);
  CloseHandle(h);
  return addr;
}
...
pe_file *pe_open(const char *path, BOOL writing) {
  ...
  if (fd == -1)
    return NULL;
  map = mmap(NULL, 4096, PROT_READ | (writing ? PROT_WRITE : 0), MAP_SHARED, fd,
             0);
  if (map == MAP_FAILED) {
    close(fd);
    return NULL;
  ...
  pef.ntheader32 =
      (PIMAGE_NT_HEADERS32)((PBYTE)pef.dosheader + pef.dosheader->e_lfanew);
  /* Sanity checks */
  ...
  pef.is_64bit =
      pef.ntheader32->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC;
  return &pef;
}
...
void get_and_set_size(const pe_file *pep, sizeof_values_t *val) {
...
static void help(FILE *f) {
  fputs(
      ...
      "Sets, clears, or displays various flags in PE files (that is, exes and "
      "dlls).\n"
      "Also sets or displays various numerical values which influence "
      "executable\n"
      "startup.  For each flag, if an argument is given, then the specified "
      "flag\n"
      "will be set or cleared; if no argument is given, then the current value "
      "of\n"
      "that flag will be displayed.  For each numerical value, if an argument "
      "is\n"
      "given, the specified value will be overwritten; if no argument is "
      "given, the\n"
      "numerical value will be displayed in decimal and hexadecimal notation.\n"
      "\n"
      "  -d, --dynamicbase  [BOOL]   Image base address may be relocated "
      "using\n"
      ...
      "handling\n"
      "                              (SEH). No SE handler may be called in "
      "this image.\n"
      ...
      "  -x, --stack-reserve [NUM]   Reserved stack size of the process in "
      "bytes.\n"
      "  -X, --stack-commit  [NUM]   Initial commited portion of the process "
      "stack\n"
      "                              in bytes.\n"
      "  -y, --heap-reserve  [NUM]   Reserved heap size of the default "
      "application\n"
      "                              heap in bytes.  Note that this value has "
      "no\n"
      "                              significant meaning to Cygwin "
      "applications.\n"
      "                              See the -z, --cygwin-heap option "
      "instead.\n"
      "  -Y, --heap-commit   [NUM]   Initial commited portion of the default\n"
      "                              application heap in bytes.  Note that "
      "this value\n"
      "                              has no significant meaning to Cygwin "
      "applications.\n"
      "                              See the -z, --cygwin-heap option "
      "instead.\n"
      "  -z, --cygwin-heap   [NUM]   Initial reserved heap size of the Cygwin\n"
      "                              application heap in Megabytes.  This "
      "value is\n"
      "                              only evaluated starting with Cygwin "
      "1.7.10.\n"
      "                              Useful values are between 4 and 2048.  If "
      "0,\n"
      "                              Cygwin uses the default heap size of 384 "
      "Megs.\n"
      "                              Has no meaning for non-Cygwin "
      "applications.\n"
      ...
      "          if 0, false, or no - indicates that the flag should be "
      "cleared\n"
      "          if not present, then display symbolicly the value of the "
      "flag\n"
      "NUM : may be any 32 bit value for 32 bit executables, any 48 bit value "
      "for\n"
      "      64 bit executables.\n"
      ...
      "For flag values, to set a value, and display the results symbolic, "
      "repeat the\n"
...
static void version(FILE *f) {
  ...
  fprintf(f, "Copyright (c) 2009, 2011  Charles Wilson, Dave Korn, Jason "
             "Tishler, et al.\n");
}
